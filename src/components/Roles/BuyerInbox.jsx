...
import notificationSound from "../../assets/sounddd.mp3";
...
const BuyerInbox = () => {
  ...
  const clickListref = useRef();
  const { userid: alsoLoggedInUserId } = useParams(); // me as buyer id
  let location = useLocation();
  let otherUserID = location.state.otherUserID;
  ...
  }, [socket, selectedChatList]); // Add socket to dependency array to ensure the effect runs when socket changes
  ...
  const sendMessage = () => {
    if (messageText) {
      socket.emit("messageSent", {
        senderid: alsoLoggedInUserId, //
        recieverId: activeChatId ? activeChatId : otherUserID,
        messageText,
        Room: Room ? Room : "",
      });
      setmessageText("");
